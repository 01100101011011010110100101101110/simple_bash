	...
	*/
	if (!strcmp(word, "|")) {
		...
		return 1;
	        ...
	        spec_sym_arr = realloc(spec_sym_arr, sizeof(int) * (2 + spec_sym_idx));
		spec_sym_arr[spec_sym_idx] = 2;
		return 1;
	} else if (!strcmp(word, "&&")) {
        	spec_sym_arr = realloc(spec_sym_arr, sizeof(int) * (2 + spec_sym_idx));
		spec_sym_arr[spec_sym_idx] = 3;
		return 1;
	} else if (!strcmp(word, ";")) {
	        spec_sym_arr = realloc(spec_sym_arr, sizeof(int) * (2 + spec_sym_idx));
		spec_sym_arr[spec_sym_idx] = 4;
		return 1;
	} else {
		return 0;	
	}
...
int process_argv(char ***data_arr) {
	int pid, status = 0;
	int run = 1;
	int fd[2];
	int *fio = NULL;
...
void main_process(char ***data_arr) {
	do {
		...
		switch (spec_sym_arr[sym_idx]) {
			case 1:			//ampersanad
			case 0:			//pipe
				suc_flag = !process_argv(data_arr);
				break;
			case -1:
				sym_idx++;		
				suc_flag = !process_argv(data_arr);
...
int main() {
	...
	for (;;) {
		spec_sym_arr = (int *) malloc(sizeof(int) * 2);
		spec_sym_arr[0] = -1;
